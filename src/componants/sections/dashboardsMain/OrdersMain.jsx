...
import { Package } from "lucide-react"; // Adjust the import path as necessary
...
const OrdersMain = () => {
  ...
  const formatStatus = (status) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await getOrders(); // Pass userId to the API call
  ...
  const displayOrders = orders.length > 0 ? orders.map(order => ({
    ...
    return (
        <div class='space-y-2 md:space-y-3 w-full p-4 md:p-0'>
